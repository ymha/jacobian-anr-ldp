...
def make_windows(
    ...
    """
    df = _add_run_col(df)
    unique_hhs = np.random.default_rng(seed).permutation(df["LCLid"].unique())
    hh_codes = {hh: i for i, hh in enumerate(unique_hhs)}
    Zs, Ys, ds, ids = [], [], [], []
...
def process(
    ...
    n_public_hh: int = N_PUBLIC_HH, seed: int = 0,
):
    tag = os.path.basename(out_dir)
    print(f"\n[{tag}] building windows ...")
    Z, Y, dates, hh_ids = make_windows(df, window, feat_col, label_cols, seed=seed)
...
def main():
    ...
    parser.add_argument("--seed",         type=int, default=0,
                        help="seed for random household shuffle before public/private split")
    ...
    if args.multi:
        ...
        process(df, args.window, args.split_date,
                ...
                n_public_hh=args.n_public_hh, seed=args.seed)
    else:
