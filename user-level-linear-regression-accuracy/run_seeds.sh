#!/usr/bin/env bash
# Run eval_regression.py with multiple seeds and save per-seed outputs.
#
# Usage:
#   bash run_seeds.sh                    # seeds 0..4, n_rounds=120, n_dates=20
...
for seed in $SEEDS; do
    ...
    echo "=== seed=${seed} → ${out} ==="
    python eval_regression.py --seed "$seed" --n_rounds 130 --n_dates 20 "${EXTRA_ARGS[@]}" | tee "$out"
done
