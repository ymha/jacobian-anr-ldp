...
def parse_result_file(path: Path) -> dict:
    """Parse one result file. Returns {eps: {mech_name: accuracy}}."""
    results = {}
    ...
    for line in lines:
        ...
        # Header row starts with "eps"
        if re.match(r"eps\b", stripped):
            ...
            header = fixed.split()  # ['eps', 'NoNoise', ...]
            continue
...
def aggregate(result_dir: Path, out_path: Path | None):
    ...
    header_line = (
        f"\nseeds={len(files)}\n"
        f"{'eps':>10}  " + "".join(f"{m:>{col_w}}" for m in all_mechs) + "\n"
