...
def make_anr_sv(W, features, bounds, Z_pub_norm,
...
def make_anr_sv_linf(W, features, bounds, Z_pub_norm,
                     lambda_n=LAMBDA_N, percentile=PERCENTILE):
    ...
    class _ANRSVLinf:
        def encode(self, z: np.ndarray, epsilon: float,
            ...
            if rng is None:
                rng = np.random.default_rng()
            z_sc = (z - mu) @ U * inv_sqrt_l
            z_cl = np.clip(z_sc, -rho, rho)
            return z_cl + rng.laplace(0, 2.0 * rho * d / epsilon, z_cl.shape)
...
def make_anr_sv_l2agm(W, features, bounds, Z_pub_norm,
...
def make_anr_sv_privunit2(W, features, bounds, Z_pub_norm,
                          lambda_n=LAMBDA_N, percentile=PERCENTILE):
    ...
    return make_anr_sv_wrap(
        lambda x: make_privunit2(x, percentile),
        W, features, bounds, Z_pub_norm, lambda_n, percentile,
    )


def make_anr_sv_cw(W, features, bounds, Z_pub_norm,
                   lambda_n=LAMBDA_N, percentile=PERCENTILE):
...
def make_anr_sv_cw_gaussian(W, features, bounds, Z_pub_norm,
                             lambda_n=LAMBDA_N, percentile=PERCENTILE, delta=DELTA):
...
def make_anr_sv_privunitg(W, features, bounds, Z_pub_norm,
                           lambda_n=LAMBDA_N, percentile=PERCENTILE):
...
def make_anr_sv_duchi(W, features, bounds, Z_pub_norm,
                      lambda_n=LAMBDA_N, percentile=PERCENTILE):
...
def make_anr_sv_piecewise(W, features, bounds, Z_pub_norm,
                           lambda_n=LAMBDA_N, percentile=PERCENTILE):
...
def make_anr_sv_wrap(inner_factory, W, features, bounds, Z_pub_norm,
                     lambda_n=LAMBDA_N, percentile=PERCENTILE):
...
def make_plan(Z_pub_norm: np.ndarray, p: int = 2,
...
def make_shifted_cm(Z_pub_norm: np.ndarray, delta: float = DELTA):
    ...
    # Rotate public data: Z_hat[i] = (1/√d) · H @ (d_vec ⊙ z[i])
    # Batch form: Z_hat = (Z * d_vec) @ H / √d  (H symmetric ⟹ H = H.T)
...
def make_privunit2(Z_pub_norm: np.ndarray, percentile: float = PERCENTILE):
...
def make_privunitg(Z_pub_norm: np.ndarray, percentile: float = PERCENTILE):
...
def _cheng_setup(W, features, bounds, Z_train_norm, percentile):
    """Cholesky whitening setup for make_cheng.

    Returns (d, mu, L, L_inv, r, P) where:
      mu:    training mean in [0,1]^d
      L:     Cholesky factor of Cov(x_train)
      L_inv: inverse of L
      r:     `percentile`-th percentile of ||h||_2 over training data
      P:     (1, d) task matrix in whitened space (W_norm @ L)
    """
    d      = len(features)
    mu     = Z_train_norm.mean(axis=0)
    Z_c    = Z_train_norm - mu
    Cov    = (Z_c.T @ Z_c) / len(Z_train_norm) + 1e-8 * np.eye(d)
    ...
    W2     = W_norm.reshape(1, -1) if W_norm.ndim == 1 else W_norm
    P      = W2 @ L  # (K, d) where K = number of outputs
    return d, mu, L, L_inv, r, P


def make_cheng(W, features, bounds, Z_train_norm, percentile=PERCENTILE):
    """Cheng et al. (2022, ICML) task-aware LDP mechanism for linear MSE settings.

    Encoder-decoder framework (Section 4.1):
      1. Whiten x via Cholesky of Cov(x_train): h = L^{-1}(x - μ).
    ...
    """
    d, mu, L, L_inv, r, P = _cheng_setup(W, features, bounds, Z_train_norm, percentile)
    ...
    _cache:    dict[float, tuple] = {}  # epsilon -> (D, Zp, E)
    _last_eps: list               = [None]
...
def build_regression_mechs(dim: int, W: np.ndarray, Z_np: np.ndarray) -> dict:
    ...
    anr_kw   = dict(W=W, features=features, bounds=bounds,
                    Z_pub_norm=Z_np, lambda_n=LAMBDA_N, percentile=PERCENTILE)
    cheng_kw = dict(W=W, features=features, bounds=bounds,
                    Z_train_norm=Z_np)
    ...
    mechs = {
        ...
        "TASK(Cheng22)":    make_cheng(**cheng_kw),
    }
