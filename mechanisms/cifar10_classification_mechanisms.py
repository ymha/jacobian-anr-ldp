...
def _anr_sv_setup(W, features, bounds, lambda_n, X_pub_norm=None):
    ...
    U  = Vt.T
    sv = sv_all[:r]
    mu = X_pub_norm.mean(axis=0) if X_pub_norm is not None else 0.5 * np.ones(d)
...
class SymClipMech:
    """[0,1]-based mechanism (Piecewise, Duchi) with configurable clip bounds.

    Clips z to [lo, ρ], scales to [0,1] → calls mechanism → scales back.
    lo defaults to -ρ (symmetric, for bipolar data); pass lo=0 for non-negative
    data so the full [0,1] input range is used instead of only [0.5, 1.0].
    ρ = L∞ norm 90th percentile of Z_tr.
    """
    def __init__(self, base_mech, rho: float, lo: float = None):
        self._enc  = base_mech.encode
        self._dec  = base_mech.decode
        self._rho  = rho
        self._lo   = -rho if lo is None else lo

    def encode(self, z: np.ndarray, eps: float,
               rng: np.random.Generator = None) -> np.ndarray:
        span  = self._rho - self._lo
        z_clip = np.clip(z, self._lo, self._rho)
        return self._enc((z_clip - self._lo) / span, eps, rng)

    def decode(self, z_enc: np.ndarray) -> np.ndarray:
        span = self._rho - self._lo
        return self._dec(z_enc) * span + self._lo
...
def make_anr_sv(W, features, bounds, X_pub_norm,
    ...
    """
    d, r, sv, U, sqrt_l, inv_sqrt_l, mu = _anr_sv_setup(W, features, bounds, lambda_n, X_pub_norm)
...
def make_shifted_cm(X_pub_norm: np.ndarray, delta: float = DELTA):
    ...
    d_vec = np.random.default_rng(0).choice(np.array([-1.0, 1.0]), size=d)  # shared public randomness, fixed seed
    H = _hadamard(d).astype(float)  # H symmetric → H = H.T

    X_hat   = (X_pub_norm * d_vec) @ H / np.sqrt(d)
    c_tilde = np.median(X_hat, axis=0)
...
def build_latent_mechs(dim: int, W: np.ndarray, Z_np: np.ndarray) -> dict:
    ...
    lo_clip: data-adaptive lower clip bound for Duchi/Piecewise (SymClipMech).
    For non-negative features (e.g. ResNet-20) this equals 0, so the [0,1]
    input range is fully used instead of [0.5, 1.0].
    For bipolar data (e.g. VAE latents) lo_clip ≈ -ρ and behaviour is unchanged.
    """
    lo    = Z_np.min(axis=0)
    hi    = Z_np.max(axis=0)
    ...
    lo_clip = float(max(Z_np.min(), -rho_linf))
    ...
    mechs = {
        ...
        "Duchi":            SymClipMech(duchi_ldp,     rho_linf, lo=lo_clip),
        "ANR-SV+Duchi":     make_anr_sv_duchi(**anr_kw),
        "Piecewise":        SymClipMech(piecewise_ldp, rho_linf, lo=lo_clip),
