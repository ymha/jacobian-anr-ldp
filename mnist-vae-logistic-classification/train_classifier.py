"""
Train VAE encoder and logistic classifier jointly end-to-end on MNIST.

Initializes encoder from a pretrained VAE checkpoint, then fine-tunes encoder
and logistic classifier together with cross-entropy loss. After training,
re-extracts latent vectors (Z_tr, Z_val, Z_te) from the updated encoder
...
class LatentLogistic(nn.Module):
...
def load_classifier(dim: int, device: str) -> nn.Module:
    ckpt  = torch.load(os.path.join(CKPT_DIR, f"latent_logistic_d{dim}.pt"),
                       map_location=device, weights_only=True)
    model = LatentLogistic(dim).to(device)
...
def train_joint(dim: int, epochs: int = 100, lr: float = 1e-3,
    ...
    clf       = LatentLogistic(dim).to(device)
    ...
    torch.save({"state_dict": clf.state_dict(), "dim": dim},
               os.path.join(CKPT_DIR, f"latent_logistic_d{dim}.pt"))
    print(f"Saved → {os.path.join(CKPT_DIR, f'latent_logistic_d{dim}.pt')}")
    return encoder, clf
